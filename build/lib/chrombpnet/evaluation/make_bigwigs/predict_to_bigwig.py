...
def main(args):
    ...
    if args.chrombpnet_model:
        ...
        atac_bw = pyBigWig.open(args.bigwig_atac)
        atac_data = data_utils.get_cts(regions_df, atac_bw, inputlen)
        atac_data = np.expand_dims(atac_data, axis=-1)

        inp = np.concatenate([seqs,atac_data], axis=-1)

        pred_logits, pred_logcts = model_chrombpnet.predict([inp],
                                          batch_size = args.batch_size,
                                          verbose=True)

        pred_logits = np.squeeze(pred_logits)

        asdf = softmax(pred_logits) * (np.expand_dims(np.exp(pred_logcts)[:,0],axis=1))
        print(asdf.shape)
        print(regions)
        ...
        if args.bigwig_dnase:
        	compare_with_observed(args.bigwig_dnase, regions_df, regions, outputlen,
