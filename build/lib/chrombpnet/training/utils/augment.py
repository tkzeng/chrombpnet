...
def random_crop(seqs, labels_atac, labels_dnase, seq_crop_width, label_crop_width, coords):
    """
    ...
    """
    #print(seqs.shape, labels_atac.shape, labels_dnase.shape, seq_crop_width, label_crop_width)

    assert(seqs.shape[1]>=seq_crop_width)
    assert(labels_atac.shape[1]>=seq_crop_width)
    assert(seqs.shape[1] - seq_crop_width == labels_atac.shape[1] - seq_crop_width)
    assert(labels_dnase.shape[1]>=label_crop_width)
    assert(seqs.shape[1] - seq_crop_width == labels_dnase.shape[1] - label_crop_width)
    ...
    return take_per_row(seqs, starts, seq_crop_width), take_per_row(labels_atac, starts, seq_crop_width), \
           take_per_row(labels_dnase, starts, label_crop_width), new_coords
